                ...
                var populatesubgrid = function (rd) {
                    ...
                    if (!ts.grid.hDiv.loading) {
                        ...
                        switch (ts.p.subgridtype) {
                            case "xml":
                                $.ajax({type: ts.p.mtype, url: ts.p.subGridUrl, dataType: "xml", data: dp, complete: function (sxml) {
                                        subGridXml(sxml.responseXML, sid);
                                        sxml = null;
                                    }});
                                break;
                            case "json":
                                $.ajax({type: ts.p.mtype, url: ts.p.subGridUrl, dataType: "text", data: dp, complete: function (JSON) {
                                        subGridJson($.parse(JSON.responseText), sid);
                                        JSON = null;
                                    }});
                                break;
                        }
