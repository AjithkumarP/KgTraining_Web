...
var createModal = function (aIDs, content, p, insertSelector, posSelector, appendsel) {
    ...
    if (appendsel === true) {
        ...
        jQuery(mw).insertBefore(insertSelector);
    }
...
var viewModal = function (selector, o) {
    ...
    if (jQuery.fn.jqm && o.jqm == true) {
        ...
        try {
            jQuery(':input:visible', selector)[0].focus();
        } catch (_) {
        }
    }
    return false;
...
function checkValues(val, valref, g) {
    ...
    if (edtrul) {
        ...
        if (edtrul.required === true) {
            if (val.match(/^s+$/) || val == "")
                return [false, nm + ": " + jQuery.jgrid.edit.msg.required, ""];
        }
        // force required
        ...
        if (edtrul.number === true) {
            if (!(rqfield === false && isEmpty(val))) {
                if (isNaN(val))
                    return [false, nm + ": " + jQuery.jgrid.edit.msg.number, ""];
            }
        }
        if (edtrul.minValue && !isNaN(edtrul.minValue)) {
            if (parseFloat(val) < parseFloat(edtrul.minValue))
                return [false, nm + ": " + jQuery.jgrid.edit.msg.minValue + " " + edtrul.minValue, ""];
        }
        if (edtrul.maxValue && !isNaN(edtrul.maxValue)) {
            if (parseFloat(val) > parseFloat(edtrul.maxValue))
                return [false, nm + ": " + jQuery.jgrid.edit.msg.maxValue + " " + edtrul.maxValue, ""];
        }
        var filter;
        if (edtrul.email === true) {
            if (!(rqfield === false && isEmpty(val))) {
                ...
                if (!filter.test(val)) {
                    return [false, nm + ": " + jQuery.jgrid.edit.msg.email, ""];
                }
            }
        }
        if (edtrul.integer === true) {
            if (!(rqfield === false && isEmpty(val))) {
                if (isNaN(val))
                    return [false, nm + ": " + jQuery.jgrid.edit.msg.integer, ""];
        ...
        if (edtrul.date === true) {
            if (!(rqfield === false && isEmpty(val))) {
                ...
                if (!checkDate(dft, val))
                    return [false, nm + ": " + jQuery.jgrid.edit.msg.date + " - " + dft, ""];
            }
        }
        if (edtrul.url === true) {
            if (!(rqfield === false && isEmpty(val))) {
                ...
                if (!filter.test(val)) {
                    return [false, nm + ": " + jQuery.jgrid.edit.msg.url, ""];
                }
            }
        }
    }
