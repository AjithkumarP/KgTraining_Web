    ...
    $.fn.jqGrid = function (p) {
        ...
        $.fn.setGridWidth = function (nwidth, shrink) {
            return this.each(function () {
                ...
                if (nwidth === grid.width) {
                    return;
                }
                else {
                    grid.width = $t.p.width = nwidth;
                }
            ...
            var ts = this,
                        ...
                        if (gxml && gl) {
                            ...
                            while (j < gl) {
                                ...
                                if (ts.p.gridview === false) {
                                    ...
                                    if (ts.p.subGrid === true) {
                                        try {
                                            $(ts).addSubGrid(ts.rows[ts.rows.length - 1], gi);
                                        } catch (e) {
                                        }
                                    }
                                    if (afterInsRow) {
                                        ts.p.afterInsertRow(rid, rd, this);
                                    }
                                    rowData = [];
                                    ari = 0;
                                }
                        ...
                        if (drows) {
                            ...
                            while (i < len) {
                                ...
                                if (ts.p.gridview === false) {
                                    ...
                                    if (afterInsRow) {
                                        ts.p.afterInsertRow(idr, rd, drows[i]);
                                    }
                                    rowData = [];
                                    ari = 0;
                                }
                    ...
                    populate = function () {
                        if (!grid.hDiv.loading) {
                            ...
                            if ($.isFunction(ts.p.datatype))
                                ts.p.datatype(gdata, "load_" + ts.p.id);
                            else
                                beginReq();
                                ...
                                case "script":
                                    $.ajax({url: ts.p.url, type: ts.p.mtype, dataType: dt == 'json' ? 'text' : dt, data: gdata,
                                        complete: function (req, st) {
                                            if (st == "success") {
                                                if (dt === "json" || dt === "script")
                                                    addJSONData($.parse(req.responseText), ts.grid.bDiv, rcnt);
                                                if (dt === "xml")
                    ...
                    setPager = function () {
                        ...
                        if (ts.p.viewrecords === true) {
                            $("td#" + pgid + "_" + ts.p.recordpos, "#" + pgcnt).append("<div class='ui-paging-info'></div>");
                        }
