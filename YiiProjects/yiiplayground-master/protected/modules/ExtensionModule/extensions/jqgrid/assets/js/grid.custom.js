        ...
        sortGrid: function (colname, reload) {
            return this.each(function () {
                ...
                if (idx != -1) {
                    ...
                    if (sort) {
                        $t.sortData("jqgh_" + colname, idx, reload);
                    }
                }
            });
        },
                                ...
                                if (this.surl) {
                                    ...
                                    $(thd).load(this.surl, {_nsd: (new Date().getTime())}, function () {
                                        if (soptions.defaultValue)
                                            $("select", this).val(soptions.defaultValue);
                                        $("select", this).attr({name: cm.name, id: "gs_" + cm.name});
                                        if (soptions.attr) {
                                            $("select", this).attr(soptions.attr);
                                        }
                                        $("select", this).css({width: "100%"});
                                        // preserve autoserch
                                        if (soptions.dataInit != null)
                                            soptions.dataInit($("select", this)[0]);
                                        if (soptions.dataEvents != null)
                                            bindEvents($("select", this)[0], soptions.dataEvents);
