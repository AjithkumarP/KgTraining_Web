<?php
/* @var $this Sem2Controller */
/* @var $model Sem2 */
/* @var $form CActiveForm */
?>

<div class="form">

    <?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'sem2-form',
	// Please note: When you enable ajax validation, make sure the corresponding
	// controller action is handling ajax validation correctly.
	// There is a call to performAjaxValidation() commented in generated controller code.
	// See class documentation of CActiveForm for details on this.
	'enableAjaxValidation'=>false,
)); ?>

    <p class="note">Fields with <span class="required">*</span> are required.</p>

    <?php echo $form->errorSummary($model); ?>

            <div class="row">
            <?php echo $form->labelEx($model,'id'); ?>
            <?php echo $form->textField($model,'id'); ?>
            <?php echo $form->error($model,'id'); ?>
        </div>

                <div class="row">
            <?php echo $form->labelEx($model,'year'); ?>
            <?php echo $form->textField($model,'year'); ?>
            <?php echo $form->error($model,'year'); ?>
        </div>

                <div class="row">
            <?php echo $form->labelEx($model,'algorithms'); ?>
            <?php echo $form->textField($model,'algorithms'); ?>
            <?php echo $form->error($model,'algorithms'); ?>
        </div>

                <div class="row">
            <?php echo $form->labelEx($model,'electronics'); ?>
            <?php echo $form->textField($model,'electronics'); ?>
            <?php echo $form->error($model,'electronics'); ?>
        </div>

                <div class="row">
            <?php echo $form->labelEx($model,'maths2'); ?>
            <?php echo $form->textField($model,'maths2'); ?>
            <?php echo $form->error($model,'maths2'); ?>
        </div>

                <div class="row">
            <?php echo $form->labelEx($model,'java'); ?>
            <?php echo $form->textField($model,'java'); ?>
            <?php echo $form->error($model,'java'); ?>
        </div>

            <div class="row buttons">
        <?php echo CHtml::submitButton($model->isNewRecord ? 'Create' : 'Save'); ?>
    </div>

    <?php $this->endWidget(); ?>

</div><!-- form -->